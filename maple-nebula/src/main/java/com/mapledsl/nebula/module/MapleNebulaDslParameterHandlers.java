...
public enum MapleNebulaDslParameterHandlers implements MapleDslParameterHandler {
    ...
    Class(Class.class, MapleDslParameterHandler.escaped()) {
        @Override
        public Object compose(@NotNull Object parameter, MapleDslConfiguration configuration) {
            final java.lang.Class<?> it = (Class<?>) parameter;
            final BeanDefinition<?> beanDefinition = configuration.beanDefinition(it);
            return beanDefinition == null ? it.getSimpleName().toLowerCase(configuration.globalLocale()) : beanDefinition.label();
        }
    },
    ...
    <R> MapleNebulaDslParameterHandlers(Class<R> parameterType, MapleDslParameterHandler delegate) {
